...
# o	Iterate over all nodes in G (not including the metadata).
# o	Use the len() and list() functions together with the .neighbors() method to calculate the total number of neighbors that node n in graph G has.
# ?	If the number of neighbors of node n is equal to m, add n to the set nodes using the .add() method.
# o	After iterating over all the nodes in G, return the set nodes.

...
# Now that you have a model with 2 outputs, compile it with 2 loss functions: 
# mean absolute error (MAE) for 'score_diff' and binary cross-entropy (also known as logloss) for 'won'. 
# Then fit the model with 'seed_diff'and 'pred' as inputs. For outputs, predict 'score_diff' and 'won'.

# This model can use the scores of the games to make sure that close games (small score diff) 
# have lower win probabilities than blowouts (large score diff).
